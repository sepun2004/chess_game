...
public class Tablero {
     ...
     *
     * @return true si el movimiento se realizó con éxito, false si falló.
     */
    public boolean moverPieza(int fO, int cO, int fD, int cD, boolean esBlanco) {

        if (!esPosicionValida(fO, cO) || !esPosicionValida(fD, cD)) {
            return false;
        }

        Pieza p = casillas[fO][cO];

        if (p == null || p.isEsBlanca() != esBlanco) {
            return false;
        }

        // Verificar si es un intento de enroque
        if (p.esRey() && Math.abs(cD - cO) == 2) {
        ...
        if (p.esMovimientoValido(fD, cD, casillas)) {
            ...
            if (capturada != null) {
                capturarPiezaEnemiga(capturada);
            }
            ...
            // Marcar que la pieza se movió (si corresponde)
            p.marcarMovimiento();
    ...
    /**
     * Registra una pieza capturada.
     *
     * @param pieza La pieza que fue capturada.
     */
    private void capturarPiezaEnemiga(Pieza pieza) {
     ...
     * @param tipoPieza Tipo de pieza destino.
     *                   ('R' Reina, 'T' Torre, 'A' Alfil, 'C' Caballo)
     *
    ...
    public boolean promoverPeon(int fila, int columna, char tipoPieza) {
        ...
        if (!p.esPeon() || !p.puedeSerPromovido()) {
            return false;
        }
     ...
     * @param esBlanca true si verificamos ataques por piezas negras,
     *                  false para blancas.
     *
    ...
    public boolean estaBajoAtaque(int fila, int columna, boolean esBlanca) {
        ...
        for (int i = 0; i < TAMANO; i++) {
            ...
            for (int j = 0; j < TAMANO; j++) {
                ...
                // Ignorar piezas propias
                if (p.isEsBlanca() == esBlanca) {
                    continue;
                }
     ...
     *
     * @param esBlanca true para el rey blanco,
     *                  false para el rey negro.
     *
     ...
     *
     * @param esBlanca true para encontrar el rey blanco,
     *                  false para el negro.
     *
        ...
        for (int i = 0; i < TAMANO; i++) {
            ...
            for (int j = 0; j < TAMANO; j++) {
                ...
                if (casillas[i][j] != null &&
                    casillas[i][j].esRey() &&
                    casillas[i][j].isEsBlanca() == esBlanca) {
     ...
     *
     * @param esBlanca true para verificar el rey blanco,
     *                  false para el negro.
     *
    ...
    public boolean esJaqueMate(boolean esBlanca) {
        ...
        // Verificar si existe algún movimiento legal
        for (int i = 0; i < TAMANO; i++) {
    ...
    public boolean puedeHacerEnroque(
        ...
        if (rey == null || rey.esHaMovido()) {
            return false;
        }

        int cTorre;

        // Determinar lado del enroque
        if (cD > cO) {
        ...
        if (p == null || !p.esTorre() || p.esHaMovido()) {
    ...
    public void hacerEnroque(int fO, int cO, int cD) {
        ...
        Pieza piezaRey = casillas[fO][cO];
        int cTorre = cD > cO ? 7 : 0;
        Pieza piezaTorre = casillas[fO][cTorre];

        // Mover rey
        int cReyDestino = cD > cO ? 6 : 2;

        casillas[fO][cReyDestino] = piezaRey;
        casillas[fO][cO] = null;

        piezaRey.mover(fO, cReyDestino);
        piezaRey.marcarMovimiento();

        // Mover torre
        int cTorreDestino = cD > cO ? 5 : 3;

        casillas[fO][cTorreDestino] = piezaTorre;
        casillas[fO][cTorre] = null;

        piezaTorre.mover(fO, cTorreDestino);
        piezaTorre.marcarMovimiento();
    }
}